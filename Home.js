  ...
  render() {
    return (
      <View style={styles.container}>
        <Text style={styles.title}>flix<Text style={{
        color: '#ff4981'
      }}>.</Text>
        </Text>
        {/*Each button will call the press function with a different movie name*/}

        <TextInput style={styles.searchInput} onChangeText={(movie) => this.setState({movie})} value={this.state.movie} onSubmitEditing={() => this.press(this.state.movie)} placeholder="Movie Search..."/>
